...
if selected == "Dashboard":
    ...
    if data:
        ...
        with col4:
            ...
            st.markdown(f"<p style='text-align:center;'>CO₂: {eco2} ppm</p>", unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)

        # Second row with more detailed information
        ...
        with col1:
            ...
            # Create a gauge chart for air quality
...
# --- Historical Data Page ---
    ...
    if data:
        ...
        if show_temp:
            ...
            # Create a temperature chart with Plotly
            fig = px.line(
                filtered_df,
                x="timestamp",
            ...
            temp_stats_cols = st.columns(4)
            with temp_stats_cols[0]: st.metric("Indoor Avg", f"{filtered_df['indoor_temp'].mean():.1f} °C")
            with temp_stats_cols[1]: st.metric("Indoor Max", f"{filtered_df['indoor_temp'].max():.1f} °C")
            with temp_stats_cols[2]: st.metric("Outdoor Avg", f"{filtered_df['outdoor_temp'].mean():.1f} °C")
            with temp_stats_cols[3]: st.metric("Outdoor Max", f"{filtered_df['outdoor_temp'].max():.1f} °C")
        ...
        if show_humidity:
            ...
            # Add reference lines for optimal humidity range
        ...
        if show_air_quality:
            ...
            # Create tabs for different air quality metrics
...
# --- Weather Forecast Page ---
    ...
    # Get location from latest data or allow user to enter it
    city_try = data[0].get("city", "Your City") if data and len(data) > 0 else "Your City"

    # Create two columns for city input and fetch button
    ...
    if fetch_forecast or 'forecast_data' in st.session_state:
        ...
        if forecast_data:
            ...
            if forecasts:
                ...
                # Create a table-like layout for today's forecast
                if today_forecasts:
                    ...
                    for i, entry in enumerate(today_forecasts):
                        with cols[i]:
                            ...
                            st.markdown(f"<p style='font-weight:bold; font-size:1.2rem;'>{time_str}</p>",
                                        unsafe_allow_html=True)
                            st.image(icon_url, width=64)
                            st.markdown(f"<p style='font-size:1.5rem; font-weight:bold;'>{temp:.1f}°C</p>",
                                        unsafe_allow_html=True)
                ...
                for i, (date, info) in enumerate(days):
                    with cols[i]:
                        ...
                        # Create daily card
                        st.markdown("<div class='forecast-card'>", unsafe_allow_html=True)
                        st.markdown(f"<p style='font-weight:bold; font-size:1.2rem;'>{day_name}</p>",
                                    unsafe_allow_html=True)
                        st.image(icon_url, width=64)
                        st.markdown(f"<p>{most_common_desc.capitalize()}</p>", unsafe_allow_html=True)
                        st.markdown(
                            f"<p style='font-size:1.3rem; font-weight:bold;'>{max_temp:.1f}°C / {min_temp:.1f}°C</p>",
                            unsafe_allow_html=True)
                        st.markdown("</div>", unsafe_allow_html=True)
...
# --- Home Control Page ---
    ...
    with col1:
        ...
        current_temp = data[0].get('indoor_temp', 22) if data and len(data) > 0 else 22
        st.markdown(
            f"<p style='text-align:center; font-size:3rem; font-weight:bold; color:#1E88E5;'>{current_temp}°C</p>",
            unsafe_allow_html=True)
        ...
        # Temperature slider
        target_temp = st.slider("", 18, 28, 22, 1, format="%d°C", key="temp_slider")
    ...
    with col2:
        ...
        current_humidity = data[0].get('indoor_humidity', 45) if data and len(data) > 0 else 45
        st.markdown(
            f"<p style='text-align:center; font-size:3rem; font-weight:bold; color:#673AB7;'>{current_humidity}%</p>",
            unsafe_allow_html=True)
        ...
        # Target humidity slider (only shown if auto mode is off)
        if not auto_mode:
            target_humidity = st.slider("Target Humidity:", 30, 70, 50, 5, format="%d%%")
    ...
    with col1:
        # CO2 gauge
        current_co2 = data[0].get('eco2', 800) if data and len(data) > 0 else 800
        fig = go.Figure(go.Indicator(
            mode="gauge+number",
            value=current_co2,
            domain={'x': [0, 1], 'y': [0, 1]},
